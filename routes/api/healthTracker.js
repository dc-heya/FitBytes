...
router.post('/api/calories', async (req, res) => {
  try {
    ...
    console.log('Total calories for the day:', totalCalories);
    res.status(201).json({ message: 'Calorie intake stored successfully' });
  } catch (error) {
...
router.post('/updateWeight', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  if (token) {
    ...
    // db.User.updateUserWeight(req,res)
  } else {
    return res.status(403).send({ success: false, msg: 'Unauthorized.' });
  }
}
...
//   const token = getToken(req.headers);
//   if (token) {
//     // Update weight for the given date in the Day database
//     const updateDayWeight = db.Day.updateWeight(req, res);

//     // Update user's weight
//     const updateUserWeight = db.User.updateUserWeight(req, res);

//     // Wait for both promises to resolve
//     Promise.all([updateDayWeight, updateUserWeight])
//       .then(results => {
//         // Handle the results if needed
//         // For example:
//         const [dayResult, userResult] = results;
//         res.status(200).json({ dayResult, userResult });
//       })
//       .catch(err => {
//         // Handle errors
//         console.error(err);
...
router.get('/disease', passport.authenticate('jwt', { session: false }), (req, res) => {
  const token = getToken(req.headers);
  if (token) {
    console.log('user is loggd in to the get diseases');
    db.Disease.getDisease(req, res);
  } else {
    return res.status(403).send({ success: false, msg: 'Unauthorized.' });
  }
}
